"""D-C export: explicit execution proxies, no hindsight labels in C."""
...
def participant_fills(frame: pd.DataFrame) -> tuple[pd.DataFrame,dict]:
    """Exclude known exchange summary legs, not general smart-contract wallets."""
    df=frame.copy()
    excluded=pd.Series(False,index=df.index)
    exchanges={str(v).lower() for v in df.get('contract',[]) if pd.notna(v)}
    # SII uses symbolic labels in this snapshot. Include explicit deployments.
    # v1: https://github.com/Polymarket/py-clob-client/blob/main/py_clob_client/config.py
    # v2: https://docs.polymarket.com/resources/contracts
    exchanges.update({'0x4bfb41d5b3570defd03c39a9a4d8de6bd8b8982e',
        '0xc5d563a36ae78145c45a50134d48a1215220f80a',
        '0xe111180000d2663c0091e4f400237545b87b996b',
        '0xe2222d279d744050d28e00520010520000310f59','0x'+'0'*40})
    ...
    return df[~excluded].copy(),{'exchange_summary_rows_removed':int(excluded.sum()),
        'filter_rule':'maker/taker matches known v1/v2 exchange, observed contract value or zero address',
...
def eligible_news(documents: list[dict],market: dict,cutoff: pd.Timestamp,*,strict: bool,
                  lookback_hours: int=72,limit: int=8) -> list[dict]:
...
class NewsIndex:
    """Precompute relevance; as-of lookup is O(log n + scanned eligible items)."""
...
def export_dc(trades: pd.DataFrame,bars: pd.DataFrame,documents: list[dict],config: dict,output: Path) -> dict:
    ...
    with gzip.open(output/'dc.jsonl.gz','wt',encoding='utf-8') as strict_file,gzip.open(output/'decision_news_candidates.jsonl.gz','wt',encoding='utf-8') as exploratory:
        # Avoid allocating a pandas frame per timestamp, preserving batch exclusion.
        for stamp,batch_iter in groupby(decisions.to_dict('records'),key=lambda r:r['timestamp']):
